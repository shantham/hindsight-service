...
```bash
cd /Users/shantham/WIGTrade/hindsight-service
./test/test-runner.sh
```
...
```bash
# Run only health and banks tests
./test/test-runner.sh health banks

# Run recall tests without cleanup
./test/test-runner.sh -s recall

# Only cleanup test data
./test/test-runner.sh -c
```

### Prerequisites

1. **Server running**: `node src/server.js`
...
| `test/test-health.sh` | Health & stats tests |
| `test/test-banks.sh` | Bank management tests |
| `test/test-memories.sh` | Memory storage tests |
| `test/test-recall.sh` | Memory recall tests |
| `test/test-context.sh` | NEW: Context features tests |
| `test/test-reflect.sh` | Reflection tests |
| `test/test-export-import.sh` | Export/import tests |
| `test/test-cleanup.sh` | Retention cleanup tests |
| `test/test-errors.sh` | Error handling tests |
